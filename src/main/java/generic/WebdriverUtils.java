...
{
	String pageText = jsObj(driver).executeScript("return document.documentElement.innerText;").toString();
	return pageText;
}
public void refreshPage(WebDriver driver)
{
	jsObj(driver).executeScript("history.go(0)");
}
public String getPageTitle(WebDriver driver)
{
	String title = jsObj(driver).executeScript("return document.title;").toString();
	return title;
}
public void flash(WebDriver driver,WebElement element)
{
	...
	for(int i=0; i<=100; i++)
	{
		changeColor(element, "rgb(0,200,0)", driver);
		changeColor(element, bgcolor, driver);
	}
}
public void changeColor(WebElement element,String color,WebDriver driver)
{
	jsObj(driver).executeScript("arguments[0].style.backgroundColor = '"+color+"'", element);
	try
	{
		Thread.sleep(20);
